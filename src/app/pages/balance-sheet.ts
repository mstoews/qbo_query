...
import { BalanceSheetColumn, BalanceSheetRow } from '../models/balance-sheet';
...
@Component({
  ...
  template: `
    <qbo-toolbar [inTitle]="'Balance Sheet'" [showRefresh]="true" [showDownload]="true" [showQuery]="true" />

    @if (loading()) {
      <div class="flex items-center justify-center p-8">
        <span class="text-surface-400">Loading balance sheet...</span>
      </div>
    } @else if (error()) {
...
export class BalanceSheetComponent implements OnInit {
  ...
  protected readonly columns = signal<BalanceSheetColumn[]>([]);
  ...
  private loadData(): void {
    ...
    this.error.set('');
    this.qboService.getBalanceSheet('2024-01-01', '2026-12-31').subscribe({
      next: (data) => {
      ...
      error: () => {
        this.error.set('Failed to load balance sheet. Retry after QBO Login.');
        this.loading.set(false);
      },
    });
  }
